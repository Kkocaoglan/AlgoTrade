"""
crypto_stream.py — Binance connection and data fetching for Crypto Module.

Completely isolated from spot (loop_trader) and VIOP systems.
Uses ccxt unified API. PAPER_MODE=True → Binance Testnet, no real orders.

CryptoWebSocket: real-time price feed via Binance combined stream.
...
def fetch_funding_rate(symbol: str) -> dict:
    """Fetch 8h perpetual funding rate from Binance futures API."""
...
def fetch_funding_rate_history(symbol: str, limit: int = 1000) -> pd.DataFrame:
    """Fetch recent 8h Binance perpetual funding history for training alignment."""
    if symbol not in FUNDING_SUPPORTED_SYMBOLS:
        return pd.DataFrame(columns=["timestamp", "funding_rate", "funding_rate_avg3", "mark_price"])
    try:
        binance_symbol = symbol.replace("/", "")
        url = f"https://fapi.binance.com/fapi/v1/fundingRate?symbol={binance_symbol}&limit={int(limit)}"
        ...
        df["mark_price"] = pd.to_numeric(df.get("markPrice", 0.0), errors="coerce")
...
class CryptoStream:
    """
    Wraps ccxt.binance for OHLCV, ticker, order placement, and balance queries.
...
class CryptoWebSocket:
    """
    Binance combined stream WebSocket for BTC/ETH/SOL real-time prices.
    ...
    WS_URL = (
        ...
        "avaxusdt@ticker/suiusdt@ticker/dotusdt@ticker/hypeusdt@ticker/"
        "ondousdt@ticker/fetusdt@ticker"
    )
    # Map Binance stream names → module symbol format
    STREAM_MAP = {
        "btcusdt@ticker":  "BTC/USDT",
        "ethusdt@ticker":  "ETH/USDT",
        "bnbusdt@ticker":  "BNB/USDT",
        "solusdt@ticker":  "SOL/USDT",
        "avaxusdt@ticker": "AVAX/USDT",
        "suiusdt@ticker":  "SUI/USDT",
        "dotusdt@ticker":  "DOT/USDT",
        "hypeusdt@ticker": "HYPE/USDT",
        "ondousdt@ticker": "ONDO/USDT",
        "fetusdt@ticker":  "FET/USDT",
    }
    MAX_RETRIES   = 5
    RECONNECT_SEC = 5
    ...
    def _on_open(self, ws) -> None:
        ...
        print("[CRYPTO WS] Baglandi — 10-coin stream aktif (BTC/ETH/BNB/SOL/AVAX/SUI/DOT/HYPE/ONDO/FET)")
    ...
    def _on_message(self, ws, raw: str) -> None:
        """Parse Binance combined stream message and update _live_prices."""
        try:
            from datetime import datetime, timezone
            msg    = json.loads(raw)
            stream = msg.get("stream", "")
            symbol = self.STREAM_MAP.get(stream)
            if symbol is None:
                return

            data       = msg.get("data", {})
            price      = float(data.get("c", 0))    # current close
...
def check_symbol_on_exchange(symbol: str) -> bool:
    """Check if symbol is listed on Binance spot. Returns True on error (fail-open)."""
    try:
        exch = ccxt.binance()
        exch.load_markets()
